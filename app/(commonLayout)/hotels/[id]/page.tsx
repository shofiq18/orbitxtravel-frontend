...
export default function HotelDetailsPage() {
  ...
  // Room options carousel scroll ref
  const roomScrollRef = React.useRef<HTMLDivElement>(null);
  ...
  return (
    <div className="w-full mx-auto px-4 sm:px-8 lg:px-16 pt-3 pb-10 details-page-wrapper">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        {/* 1. Left Side: Host, Details, Amenities & Available Room Categories (Order 1) */}
          ...
          <div className="border-t border-border-custom pt-6 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-text-light">Browse room categories and select your stay layout.</p>
              </div>

              {/* Navigation Arrows for Room Options */}
              {hotel.rooms && hotel.rooms.length > 1 && (
                <div className="flex items-center space-x-2">
                  <button
                    type="button"
                    onClick={() => scrollRooms("left")}
                    className="w-8 h-8 rounded-full border border-border-custom bg-bg-secondary hover:bg-bg-primary text-text-primary flex items-center justify-center transition shadow-xs cursor-pointer"
            ...
            {(!hotel.rooms || hotel.rooms.length === 0) ? (
              ...
              <div
                ref={roomScrollRef}
                className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pt-2 overflow-x-auto lg:overflow-x-visible pb-2 scrollbar-none"
              >
                {hotel.rooms.map((room: any) => (
                  <div
                    key={room.id}
                    onClick={() => setSelectedDetailRoom(room)}
                    className="border border-border-custom bg-bg-primary flex flex-col justify-between rounded-none transition hover:border-text-light cursor-pointer group hover:shadow-md duration-300 min-w-[260px] lg:min-w-0"
                  >
                    <div>
                      {/* Room Photo Header */}
        ...
        {/* 2. Right Side: Sticky Booking Panel & Policies (Order 2 on Mobile right after Room Options!) */}
        <div className="lg:col-span-1 space-y-6 relative order-2 lg:order-2">
          ...
          <div className="bg-bg-primary border border-border-custom p-6 shadow-xl space-y-5 rounded-none lg:sticky lg:top-24">
            ...
            {hotel.rooms && hotel.rooms.length > 0 && (
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  onChange={(e) => {
                    const found = hotel.rooms.find((r: any) => r.id === e.target.value);
                    if (found) setSelectedRoom(found);
                  }}
                  className="w-full bg-bg-secondary border border-border-custom p-3 text-xs font-semibold text-text-primary outline-none cursor-pointer"
                >
                  <option value="" disabled>Choose a room category</option>
                  {hotel.rooms.map((r: any) => (
                    <option key={r.id} value={r.id}>
                      {r.type} — BDT {r.b2cPrice.toLocaleString()} / night
                    </option>
                  ))}
                </select>
              </div>
            ...
            {/* Room quantity count selector */}
            {selectedRoom && remainingAvailableRooms > 0 && (
              <div className="border border-border-custom p-3 space-y-2 bg-bg-secondary/40">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-[10px] text-text-light">({remainingAvailableRooms} left)</span>
                </div>
              </div>
            )}
            ...
            <div className="border border-border-custom rounded-none divide-y divide-border-custom overflow-hidden bg-bg-secondary relative">
              ...
              {/* Guest Count Field */}
              <div className="p-3 space-y-1 relative cursor-pointer" onClick={() => setIsGuestDropdownOpen(!isGuestDropdownOpen)}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-bold text-text-light">{isGuestDropdownOpen ? "▲" : "▼"}</span>
                </div>
              </div>
            </div>
            ...
            {isGuestDropdownOpen && (
              <div className="border border-border-custom bg-bg-primary p-4 space-y-4 shadow-xl relative z-30">
                {/* Adults */}
                <div className="flex justify-between items-center">
                  <div>
                    <h5 className="text-xs font-bold text-text-primary">Adults</h5>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <button
                      type="button"
                      disabled={adults >= currentMaxCapacity}
                      onClick={(e) => {
                        e.stopPropagation();
                        setAdults(adults + 1);
                      }}
                ...
                <div className="flex justify-between items-center border-t border-border-custom pt-3">
                  <div>
                    <h5 className="text-xs font-bold text-text-primary">Children</h5>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <button
                      type="button"
                      disabled={adults + children >= currentMaxCapacity}
                      onClick={(e) => {
                        e.stopPropagation();
                        setChildren(children + 1);
                      }}
                ...
                <div className="flex justify-between items-center border-t border-border-custom pt-3">
                  <div>
                    <h5 className="text-xs font-bold text-text-primary">Infants</h5>
            ...
            {!checkInDate || !checkOutDate ? (
              <button
                onClick={() => toast.success("Please select stay dates in Check-in / Checkout to unlock pricing!")}
              ...
              <button disabled className="w-full bg-gray-400 text-white font-bold py-3 text-xs tracking-wider uppercase rounded-none cursor-not-allowed">
                Sold Out
              </button>
            ) : (
              <button
            ...
            {checkInDate && checkOutDate && (
              <div className="border-t border-border-custom pt-4 space-y-2.5 text-xs text-text-secondary">
                <div className="flex justify-between font-medium">
                  <span>
                    BDT {(isTourOrganizer && selectedRoom?.b2bPrice ? selectedRoom.b2bPrice : (selectedRoom?.b2cPrice || hotel.rooms?.[0]?.b2cPrice || 0)).toLocaleString()} x {nights} night{nights > 1 ? "s" : ""} x {roomQuantity} room{roomQuantity > 1 ? "s" : ""}
                  </span>
                  <span>
                    BDT {(
                ...
                <div className="flex justify-between font-bold text-text-primary pt-2 border-t border-border-custom/50 text-sm">
                  <span>Total (BDT)</span>
                  <span className="text-[#0061AA]">
                    BDT {
                      (
                        (isTourOrganizer && selectedRoom?.b2bPrice
                          ? selectedRoom.b2bPrice
          ...
          <div className="bg-bg-secondary border border-border-custom p-6 space-y-4 rounded-none">
            ...
            <div className="space-y-3 text-xs text-text-secondary leading-relaxed font-semibold">
              <p><strong>Check-in:</strong> {hotel.checkInTime || "14:00"} onwards.</p>
              <p><strong>Check-out:</strong> {hotel.checkOutTime || "12:00"} noon.</p>
              <p><strong>Cancellation:</strong> 48h prior full refund.</p>
            </div>
          </div>

        </div>

        {/* 3. Guest Reviews Option Section (Order 3 on Mobile, lg:col-span-2 on Desktop!) */}
        <div className="lg:col-span-2 border-t border-border-custom pt-8 space-y-8 order-3 lg:order-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-8 gap-6 border-y border-border-custom py-8 text-xs font-semibold text-text-secondary">
            ...
            <div className="flex flex-col justify-between space-y-2 border-r border-border-custom/50 pr-2">
              <div>
                <p className="text-[10px] font-bold text-text-light uppercase tracking-wider">Cleanliness</p>
                <p className="text-base font-extrabold text-text-primary mt-1">4.9</p>
              </div>
              <Sparkles className="h-5 w-5 text-text-light" />
            </div>

            {/* Column 3: Accuracy */}
            <div className="flex flex-col justify-between space-y-2 border-r border-border-custom/50 pr-2">
              <div>
                <p className="text-[10px] font-bold text-text-light uppercase tracking-wider">Accuracy</p>
                <p className="text-base font-extrabold text-text-primary mt-1">5.0</p>
              </div>
              <Check className="h-5 w-5 text-text-light" />
            </div>

            {/* Column 4: Check-in */}
            <div className="flex flex-col justify-between space-y-2 border-r border-border-custom/50 pr-2">
              <div>
                <p className="text-[10px] font-bold text-text-light uppercase tracking-wider">Check-in</p>
                <p className="text-base font-extrabold text-text-primary mt-1">4.9</p>
              </div>
            ...
            <div className="flex flex-col justify-between space-y-2 border-r border-border-custom/50 pr-2">
              <div>
                <p className="text-[10px] font-bold text-text-light uppercase tracking-wider">Host Support</p>
                <p className="text-base font-extrabold text-text-primary mt-1">5.0</p>
              </div>
              <MessageSquare className="h-5 w-5 text-text-light" />
            </div>

            {/* Column 6: Location */}
            <div className="flex flex-col justify-between space-y-2 border-r border-border-custom/50 pr-2">
              <div>
                <p className="text-[10px] font-bold text-text-light uppercase tracking-wider">Location</p>
                <p className="text-base font-extrabold text-text-primary mt-1">4.8</p>
              </div>
            ...
            <div className="flex flex-col justify-between space-y-2">
              <div>
                <p className="text-[10px] font-bold text-text-light uppercase tracking-wider">Value Rate</p>
                <p className="text-base font-extrabold text-text-primary mt-1">4.9</p>
              </div>
              <Tag className="h-5 w-5 text-text-light" />
            </div>
          ...
          {/* Reviews Grid */}
          {reviews.length === 0 ? (
            <div className="text-center py-8 bg-bg-secondary border border-dashed border-border-custom space-y-2">
              <p className="text-sm font-semibold text-text-primary">No Guest Reviews Yet</p>
              <p className="text-xs text-text-light">Be the first guest to share your stay experience at this property!</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {reviews.slice(0, 6).map((rev: any) => {
                const isExpanded = expandedReviews[rev.id];
                const rawComment = rev.comment || "Great stay!";
                const formattedComment = formatCommentText(rawComment);
                const isLong = formattedComment.length > 180;

                return (
                  <div key={rev.id} className="space-y-3 p-5 border border-border-custom/60 bg-bg-primary rounded-none">
                    <div className="flex items-center space-x-3">
                      <img
                        src={rev.user?.profileImage || rev.user?.avatar || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=120&auto=format&fit=crop&q=80"}
                        alt={rev.user?.fullName || "Guest"}
                        className="w-10 h-10 rounded-full object-cover border border-border-custom"
                      />
                      <div>
                        <h5 className="font-bold text-text-primary text-xs">{rev.user?.fullName || "Verified Guest"}</h5>
                        <div className="flex items-center space-x-1 text-[10px] text-text-light font-medium">
                    ...
                    <p className="text-xs text-text-secondary leading-relaxed font-medium">
                      {isLong && !isExpanded ? `${formattedComment.slice(0, 180)}...` : formattedComment}
                    </p>

                    {isLong && (
                      <button
                        onClick={() => toggleExpandReview(rev.id)}
                        className="text-xs font-bold text-text-primary underline cursor-pointer hover:text-theme-primary block pt-1"
                      >
          ...
          {reviews.length > 6 && (
            <button
              ...
              className="border border-border-custom bg-bg-secondary text-text-primary hover:bg-opacity-80 px-6 py-3 text-xs font-bold transition rounded-none cursor-pointer"
            >
              Show all {reviews.length} reviews
            </button>
          )}

          {/* Post a Review Form Block */}
          {user && (
            <div className="border border-border-custom p-6 bg-bg-secondary space-y-4 rounded-none">
              <h4 className="font-bold text-text-primary text-sm uppercase tracking-wider">Leave a Review for this Hotel</h4>
              <form onSubmit={handleReviewSubmit} className="space-y-4">
                
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-text-secondary block">Rating</label>
